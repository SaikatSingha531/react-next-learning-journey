    ...
    return (
        <>
            <div className='flex flex-col justify-center items-center h-screen gap-5'>
                <h1 className='text-4xl pr-5 font-bold'>Scale Your Time With This Stopwatch</h1>
                <h1 className='text-3xl'>{timer.hour} : {timer.mint} : {timer.sec} : {timer.millisec}</h1>
                <div className="flex gap-4 mt-4">
                    <button
                        onClick={start}
                        className={`${status === null ? "px-6 py-3 bg-green-600 text-white font-semibold rounded-full shadow-md hover:bg-green-700 hover:shadow-lg transition active:scale-95" : "px-6 py-3 bg-red-600 text-white font-semibold rounded-full shadow-md hover:shadow-lg transition active:scale-95"}`}
